    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('confirmPurchase').addEventListener('click', function() {
                ...
                // ส่งคำขอไปยัง process_order.php
                ...
                .then(data => {
                    // แสดงผลตอบกลับจาก server
                    if (data.success) {
                        alert(data.message);
                        $('#sizeModal').modal('hide'); // ปิด modal เมื่อสำเร็จ
                    } else {
                        alert(data.message);
                    }
                })
                ...
                <div class="container cart-container">
                    <div class="cart-body">
                        <table class="table table-hover cart-table">
                            ...
                            <tbody id="cartItems">
                                <!-- สินค้าในตะกร้าจะถูกโหลดเข้ามาในส่วนนี้โดยอัตโนมัติ -->
                            </tbody>
                        </table>
...
<script>
// เมื่อคลิกที่ปุ่มเปิดหน้าตะกร้าสินค้า
document.getElementById('openCartModal').addEventListener('click', function (e) {
    e.preventDefault();
    $('#cartModal').modal('show');
    loadCartItems();  // เรียกฟังก์ชันโหลดข้อมูลตะกร้าสินค้า
});

// ฟังก์ชันโหลดข้อมูลสินค้าจากตะกร้าจากเซิร์ฟเวอร์
function loadCartItems() {
    fetch('fetch_cart_items.php')  // ใช้ `fetch_cart_items.php` ในการดึงข้อมูลตะกร้าเป็น JSON
        .then(response => response.json())
        .then(data => {
            let cartHTML = '';
            let totalAmount = 0;

            // วนลูปสร้าง HTML ของสินค้าในตะกร้า
            data.forEach(item => {
                const itemTotal = item.price * item.quantity;
                totalAmount += itemTotal;

                cartHTML += `
                    <tr data-orderitem-id="${item.orderItemsID}">
                        <td>${item.prodName}</td>
                        <td>${item.size}</td>
                        <td>฿${item.price}</td>
                        <td>
                            <button class="btn btn-sm btn-outline-secondary" onclick="updateQuantity(${item.orderItemsID}, -1)">-</button>
                            ${item.quantity}
                            <button class="btn btn-sm btn-outline-secondary" onclick="updateQuantity(${item.orderItemsID}, 1)">+</button>
                        </td>
                        <td>฿${itemTotal}</td>
                        <td>
                            <button class="btn btn-sm btn-danger" onclick="deleteCartItem(${item.orderItemsID})">Delete</button>
                        </td>
                    </tr>
                `;
            });

            document.getElementById('cartItems').innerHTML = cartHTML;  // แสดงรายการสินค้าในตะกร้า
            document.getElementById('totalAmount').textContent = totalAmount.toFixed(2);  // อัปเดตราคารวมทั้งหมด
        });
}

// ฟังก์ชันอัปเดตจำนวนสินค้าในตะกร้า
function updateQuantity(orderItemsID, change) {
    fetch('update_cart_item.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({ orderItemsID, change })
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
            loadCartItems();  // โหลดรายการสินค้าใหม่เพื่ออัปเดตการเปลี่ยนแปลง
        } else {
            alert(data.message);
        }
    });
...
function deleteCartItem(orderItemsID) {
    fetch('delete_cart_item.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({ orderItemsID })
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
